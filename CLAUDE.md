...
- Researcher proposal workspace hardcodes document requirement stage='pre' (from the pre-proposal-only slice: `app/dashboard/proposals/[id]/page.tsx` fetch and the submit action's required-document recheck in `app/dashboard/proposals/actions.ts`). Before the full-proposal researcher flow works, derive the stage from the proposal's type: 'pre' → 'pre'; 'full'/'continuation'/'off_cycle' → 'full' (same mapping used by reviews, see `stageForProposalType` in `lib/reviews.ts`).
