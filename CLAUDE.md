...
- Researcher profile management: an approved researcher currently has NO way to update their profile or replace their CV — the CV upload lives on /pending, which they stop seeing once approved. Since the CV is snapshotted into each proposal at submission, a stale CV propagates into every future submission. Needs a profile page (name, contact info, institution, CV replace) reachable from the researcher dashboard.
